...
assert.match(dock, /layoutActions\.length[\s\S]*ConversationActions[\s\S]*className=\{styles\.content\}/, 'Todo actions remain outside the composer content slot');
...
assert.match(todoStyles, /\.taskChip\[data-state='completed'\][\s\S]*min-height: 28px;/, 'completed Todo uses the compact presentation');
