...
export interface CanonicalConversationTextDelta { messageId: string; turnId?: string; partType: 'text' | 'reasoning'; delta: string; contentLength: number; createdAt: number }
export interface CanonicalConversationEventsResponse { schemaVersion: typeof CANONICAL_CONVERSATION_SCHEMA_VERSION; sessionId: string; afterCursor: string; cursor: string; events: CanonicalConversationEntityEvent[]; deltas?: CanonicalConversationTextDelta[]; snapshotRequired?: boolean; reason?: string }
...
export interface CanonicalMessage extends CanonicalEntityMeta { role: string; phase?: CanonicalMessagePhase; assistantStepId?: string; status: string; content?: string; contentLength?: number; contentTruncated?: boolean; reasoningContent?: string; reasoningContentLength?: number; reasoningContentTruncated?: boolean; clientRequestId?: string; error?: string }
...
export interface CanonicalToolCall extends CanonicalEntityMeta { messageId?: string; assistantStepId?: string; parentToolCallId?: string; roundId?: string; name: string; source: string; kind?: string; status: string; inputPreview?: string; inputByteLength?: number; inputTruncated?: boolean; inputRef?: string; commandPreview?: string; commandByteLength?: number; commandTruncated?: boolean; commandRef?: string; targets?: string[]; workingDir?: string; risk?: string; resultIds?: string[]; startedAt?: number; finishedAt?: number; exitCode?: number; error?: string }
export interface CanonicalToolResult extends CanonicalEntityMeta { toolCallId: string; ordinal: number; status: string; contentPreview?: string; contentByteLength?: number; contentTruncated?: boolean; errorPreview?: string; errorByteLength?: number; errorTruncated?: boolean; outputRefs?: string[]; artifactRefs?: string[]; diffRefs?: string[]; deliveredToModel?: boolean }
