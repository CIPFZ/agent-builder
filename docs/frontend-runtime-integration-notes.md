# Frontend / Runtime Integration Notes

Status: active notes for current Agent Builder frontend/runtime integration.
...
preview, even though the runtime API returned provider data.
...
When a frontend/runtime feature looks empty or inactive:

1. Verify the runtime API directly.
...
Frontend/runtime rules for any future Run work:
...
Frontend/runtime rules remain unchanged:
...
does not expose a frontend/runtime Run contract yet.
...
without changing the frontend/runtime source-of-truth boundary.
...
frontend/runtime source-of-truth boundaries after restart-style recovery.
...
Phase 9.3 accepts the minimal durable Run envelope as a workbench/runtime
...
Phase 10.1 accepts a runtime migration design for Run transition history only.
...
Phase 10.2 adds only the runtime transition-history migration and narrow store.
...
Phase 10.3 accepts where runtime transition-history writes may be wired later.
...
Phase 10.4 wires runtime transition-history writes into accepted runtime paths.
...
- Transition rows are runtime audit evidence only and are not lifecycle,
...
Phase 10.12 accepts transition history as a runtime audit foundation and
...
Phase 11.1 hardens runtime Run detail reads after projection reconciliation.
...
- Run list rows should be trusted only after runtime reconciliation from
...
- Frontend list consumers should continue to use API DTO refreshes as the
  source of truth.
...
- Persisted Run detail/list reads are reconciled API DTOs, not React-owned
  state.
...
- Phase 12.1 should add runtime ownership/link stability coverage, not frontend
  adapter or React changes.

## 2026-06-09: Phase 12.1 Run Ownership Preflight Coverage

Phase 12.1 adds workbench-only Run ownership/link stability coverage.

Frontend boundary:

- `turn_started` transition audit evidence now requires the runtime
...
Phase 12.2 should add workbench-only cancellation ownership coverage.
...
- Runtime cancellation ownership coverage proves `CancelTurn` preserves Run
...
Phase 12.3 should add workbench-only startup recovery ownership coverage.

Frontend boundary:

- Startup recovery must refresh API DTOs after stale evidence is
  terminalized.
...
- Runtime recovery coverage proves Run links survive startup-style recovery,
...
Phase 12.4 should add workbench-only explicit checkpoint resume ownership
coverage.
...
- Runtime checkpoint resume coverage proves transition audit requires a
...
- Phase 12 is accepted as runtime Run ownership contract hardening only.
...
- The current UI must keep treating API DTO refreshes as the source of
  truth.
...
- The runtime now has a read-only internal preflight DTO proving whether a
...
- The scheduler preflight remains internal to the workbench.
...
- No scheduler plan HTTP, Wails, generated binding, adapter, or React surface is
  added in this gate.
- A future scheduler plan DTO may be used as read-only runtime planning
...
- Plan item executability must be derived from the runtime Phase 13.1
...
- Plan refresh targets are descriptive runtime fields only. They do not hydrate
  lifecycle, timeline, diagnostics, artifacts, interrupted recovery,
  checkpoint, permission, MCP auth, or MCP elicitation state.
- `canSchedule` remains runtime preflight output, not frontend state.

Phase 14.1 result:

- The runtime can build read-only scheduler plan evidence for a Run/turn or
  checkpoint request.
...
Phase 14.2 accepts the internal scheduler plan DTO and keeps it workbench-only.
...
- The first worker is runtime-internal and still entered through the existing
...
- The accepted first worker shape is a foreground runtime delegate around the
...
- Runtime `Chat` now requires the internal scheduler plan/preflight before
...
- The next relevant boundary is checkpoint-resume runtime hardening, because
...
Phase 16 adds runtime coverage for explicit checkpoint resume under the
...
- Checkpoint planning stays non-executable until an explicit resumed turn
  exists.
- The explicit resumed turn uses the same runtime foreground delegate/preflight
...
- The next boundary is runtime task scheduling design, not frontend exposure.
...
Phase 17 defines runtime task scheduling ownership only.
...
Phase 17.1 adds workbench-only read-only task plan/preflight coverage.

Frontend boundary:

- No task scheduler execution, scheduler transport, adapter method, or Run
  management UI is added.
- Task plan fields are runtime read-only contract evidence, not frontend state.
...
- Task scheduler plan remains runtime-internal and read-only.
...
Phase 18 defines runtime task cancellation ownership only.
...
Phase 18.1 adds runtime contract coverage for the accepted cancellation
boundary.
...
Phase 18.2 accepts the runtime task cancellation ownership contract.
...
- The next frontend/runtime risk to design is task scheduler execution
...
Phase 19 defines the frontend/runtime boundary for future scheduler-owned task
...
- Phase 19.1 should add runtime contract coverage before any transport or UI
  exposure is considered.

## 2026-06-10: Phase 19.1 Task Scheduler Execution Delegate Contract Coverage

Phase 19.1 adds workbench-only task delegate contract coverage.
...
Phase 19.2 accepts the runtime rejection-only delegate contract.

Frontend boundary:

- Task scheduler plan items remain non-executable and runtime-internal.
...
- The next frontend/runtime risk is exposing any executable task plan path; it
...
Phase 20.1 adds runtime coverage for the Phase 20 foreground schedulability
flip.
...
- `runtimeRunSchedulerDelegateTaskTurn(...)` remains runtime-internal.
...
- Frontend exposure remains rejected until runtime idempotency, cancellation,
...
- Runtime now has an internal workbench-only
...
- The workbench-only execute contract is accepted as the future foreground
...
- The child-agent foreground runner is accepted as a runtime-internal design
  direction only.
...
- No execute transport, generated binding, adapter method, React control, or
  frontend-owned task lifecycle state is accepted until the runtime runner
  contract is implemented and reviewed.

Phase 22.6 note:

- The runtime now has an internal, test-injectable child-agent runner contract
...
- The runtime runner contract is accepted as internal-only.
...
- Frontend and transport still have no execute action. The eventual adapter
  must first prove runtime cancellation, permission/MCP, terminal evidence, and
  completion-only artifact semantics before any browser/Wails affordance is
  considered.

Phase 23.1 note:

- The coordinator started-task execution contract exists in runtime/internal
  code only.
...
- There is still no browser/Wails/React execute path. Any future transport
  exposure remains blocked until runtime wiring proves durable prompt sourcing,
...
- Real runtime/coordinator executor installation is design-only.
- There is still no execute transport or frontend action; future runtime work
...
- The internal workbench/runtime executor wiring is accepted, but frontend
...
- Internal runtime/coordinator runner smoke is validated without changing
  frontend behavior.
...
- The internal runtime runner is accepted for a future transport exposure
  design gate only.
...
- Runtime readiness is covered without secrets or `ensureStarted` bypasses, but
  ...
  is accepted.
- The next frontend/runtime boundary is a Browser Scheduler Click Harness
...
- The next frontend/runtime gap is worker-completion evidence: browser
...
- Loopback provider-backed child-agent execution smoke is implemented in
  `internal/runtime`.
- The test reaches scheduler execute, the installed runtime runner, runtime
...
- `npm run smoke:phase331` validates visible browser Execute click through the
  real scheduler execute route, installed runtime runner, real coordinator
...
- The frontend should continue to render Run/cancellation/interrupted/
  ...
  reads while runtime ownership boundaries are reviewed.

Phase 41.1 note:

- Runtime Run ownership is split intentionally: persisted identity/session
...
- Runtime tests now harden the persisted Run ownership matrix around bounded
...
- The next runtime boundary is persisted Run status authority design.
- Frontend integration remains read-only DTO based; no frontend Run state or
  Run UI ownership is accepted.

Phase 42 note:

- Persisted Run status authority remains a runtime contract. Frontend code must
...
- Runtime status-authority tests were hardened around empty projections and
...
- The next runtime boundary is an explicit Run status writer design gate.
...
- The proposed explicit Run status writer is workbench-only.
...
- Runtime Run status writes now have a narrow helper for the first rollout
  call sites.
...
- Runtime helper rollout now covers foreground task-start active status writes.
...
- Runtime terminal task lifecycle now refreshes persisted Run status through
...
- Explicit Run status writer rollout is accepted on the workbench.
...
- Runtime restart/read smoke now covers explicit Run status writer paths.
...
- Phase 44 runtime smoke is accepted.
- Next validation should prove HTTP/adapter rereads expose the same runtime
...
- Frontend Run status ownership remains API DTO reread-only.

Phase 47 note:

- Run status writer workstream is closed with runtime, HTTP, and adapter
  reread coverage.
- Future persisted Run authority work remains runtime design-gated; frontend
  must continue to treat runtime DTOs as the status source.

Phase 48 note:

- Persisted Run authority is design-only and runtime/read-only scoped.
...
- Runtime tests now cover persisted Run identity/session/summary read
  authority.
...
- The accepted next runtime scope is a summary-only persisted Run read DTO/API
  contract.
...
- Runtime now exposes summary-only persisted Run read routes.
...
- Runtime tests now verify summary DTOs cannot leak checkpoint markers or
...
- Runtime marker-only DTO contract types and tests now exist.
...
- Next work remains workbench-only helper coverage; no transport or frontend
  marker surface is accepted yet.

Phase 50.5 note:

- Runtime-only checkpoint marker read helpers are covered.
- No HTTP, Wails, frontend adapter, `WorkbenchViewModel`, or UI surface was
  added for markers.

Phase 50.6 note:

- Runtime-only checkpoint marker helpers are accepted.
...
- Runtime now has read-only HTTP marker routes with contract coverage.
...
- Future UI changes should start from concrete user-visible workflows and must
  keep runtime runtime DTO rereads as the source of truth.
