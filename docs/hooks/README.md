...
the agent lifecycle, allowing you to both build on top of Agent Builder, customize
...
our **project-level** `agent-builder.json`. Relative paths like `./no-haskell.sh` work
here because the project root is your working directory. If you're configuring
a global hook (`~/.config/agent-builder/agent-builder.json`), use an absolute path instead.
...
# ($AGENT_BUILDER_TOOL_INPUT_COMMAND) to grep and match on a regexp.
if echo "$AGENT_BUILDER_TOOL_INPUT_COMMAND" | grep -qE '(^| )((ghc|cabal|stack)(\.exe)?)( |$)'; then
...
Hooks run through Agent Builder's embedded POSIX shell (`mvdan.cc/sh`) — the same
...
- **Permissive shebang fallback**: if the absolute path in a shebang
  doesn't exist (e.g. `#!/bin/bash` on Windows), Agent Builder falls back to a
  ...
  "no opinion".
- **Environment**: every hook sees `AGENT_BUILDER=1`, `AGENT=agent-builder`, and
  `AI_AGENT=agent-builder` on top of the `AGENT_BUILDER_*` hook-specific variables. These
  ...
  both contexts.
- **Timeout behavior**: when a hook exceeds its timeout, Agent Builder cancels the
  context and waits a short grace period (~1s) for the interpreter to
  yield. If the hook still hasn't returned, Agent Builder abandons it, logs a
...
Hooks can be added to your `agent-builder.json` (or `.agent-builder.json`) at both the global
...
> work fine in project-level `agent-builder.json` because the project root is also
> your working directory. For **global** config (`~/.config/agent-builder/`),
> however, you must use either an absolute path or an inline command:
>
> ```jsonc
> // Global ~/.config/agent-builder/agent-builder.json
> {
>   "hooks": {
>     "PreToolUse": [
>       {
>         "command": "/home/you/.config/agent-builder/hooks/no-haskell.sh"
...
3. Runs all matching hooks **in parallel** through Agent Builder's embedded POSIX
...
| ---------------------------- | ---------------------------------------------- |
| `AGENT_BUILDER`                      | Always `1` when running under Agent Builder.           |
| `AGENT`                      | Always `agent-builder`.                                |
| `AI_AGENT`                   | Always `agent-builder`.                                |
| `AGENT_BUILDER_EVENT`                | The hook event name (e.g. `PreToolUse`).       |
| `AGENT_BUILDER_TOOL_NAME`            | The tool being called (e.g. `bash`).           |
| `AGENT_BUILDER_SESSION_ID`           | Current session ID.                            |
| `AGENT_BUILDER_CWD`                  | Working directory.                             |
| `AGENT_BUILDER_PROJECT_DIR`          | Project root directory.                        |
| `AGENT_BUILDER_TOOL_INPUT_COMMAND`   | For `bash` calls: the shell command being run. |
| `AGENT_BUILDER_TOOL_INPUT_FILE_PATH` | For file tools: the target file path.          |

The `AGENT_BUILDER`, `AGENT`, and `AI_AGENT` markers are also set by the `bash`
tool, so a script can detect "am I running under Agent Builder?" the same way in
either context.
...
Hooks communicate back to Agent Builder via **exit code** and `stdout`/`stderr`. The
...
deterministic based on the order hooks appear in `agent-builder.json`.
...
If a hook exceeds its timeout, Agent Builder cancels its context and treats the
...
if echo "$AGENT_BUILDER_TOOL_INPUT_COMMAND" | grep -qE 'rm\s+-(rf|fr)\s+/'; then
...
case "$AGENT_BUILDER_TOOL_INPUT_COMMAND" in
...
if [[ "$AGENT_BUILDER_TOOL_INPUT_FILE_PATH" == *.go ]]; then
...
```jsonc
{ "command": "echo \"$(date -Iseconds) $AGENT_BUILDER_TOOL_NAME\" >> ./tools.log" }
```
...
Agent Builder hooks are broadly compatible with [Claude Code
...
hooks run under Agent Builder unchanged. This document covers the Agent Builder-specific API
only — anything not documented here isn't guaranteed to work.

One intentional divergence: Agent Builder treats `updated_input` as a shallow-merge
