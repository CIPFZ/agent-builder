# Agent Builder 与 Claude Code 全模块差距梳理

本文基于当前 `main` 分支的 Agent Builder 代码，以及本地 Claude Code 快照：

```text
C:\Users\ytq\work\ai\myclaw\claude-code
```

目标不是照搬 Claude Code，而是识别 Claude Code 已经沉淀出的 agent runtime primitives，并判断哪些应该在 Agent Builder 中补齐，以支撑“以 Agent Builder 为底座、以 Claude Code 为设计蓝图、最终转为 Codex 类客户端”的方向。

## 总体结论

Agent Builder 是一个干净、可改造的 Go agent runtime 底座，但当前仍偏向“单主 agent + tools + session + TUI/API”的结构。Claude Code 已经是完整产品级 agent runtime：权限策略、工具调度、上下文经济、子 agent 生命周期、记忆、远程/SDK/结构化 IO、插件、feature gate、telemetry、恢复机制都更成熟。

两者差距不是某几个工具缺失，而是运行时层级差距。

Agent Builder 当前主线已经走在正确方向上：

- 保留 Agent Builder Go runtime 作为真实执行层。
- 用 Wails v3 做桌面薄壳。
- 用 React 做客户端交互层。
...
- Go runtime：`internal/workbench`、`internal/app`、`internal/agent`、`internal/agent/tools`
...
Claude Code 的模块拆分更偏完整产品运行时；Agent Builder 的模块更简洁，更适合作为 Go runtime 底座继续演进。

## 全模块对照

| 领域 | Agent Builder 当前 | Claude Code 当前 | 差距等级 |
| --- | --- | --- | --- |
...
| Shell 安全 | Agent Builder shell + permission prompt | Bash/PowerShell parser、dangerous pattern、subcommand analysis、read-only validation、sandbox decision | 极大 |
...
| UI | Agent Builder TUI + Agent Builder React/Wails 初版 | Ink REPL 深度状态机、permissions UI、agent panels、diff、teams、bridge/remote UI | 很大 |
...
Agent Builder 的主执行路径集中在：

- `internal/agent/agent.go`
- `internal/agent/coordinator.go`
- `internal/workbench/agent.go`
...
Agent Builder 当前工具是 Go 函数式工具集合，能力已经不少：
...
Agent Builder 当前权限模型在 `internal/permission/permission.go`：
...
Agent Builder 当前没有 Claude Code 级别 plan mode。Claude Code 的 plan mode 不是 UI 标签，而是 runtime policy：
...
Agent Builder 有 portable shell 和 background shell，适合做本地执行。但安全策略较粗。
...
Agent Builder 已有 `internal/agent/agent_tool.go`：
...
Agent Builder 当前 prompt assembly 在：
...
Agent Builder 使用 SQLite 保存 session/message/todos，结构更数据库化。
...
Agent Builder 的 SQLite 是优势，但需要补足 equivalent concepts：
...
Agent Builder 已有 `internal/server` workspace API，Agent Builder 又新增了 `internal/runtimeapi` 和桌面 local HTTP/SSE。
...
```text
React UI -> AgentRuntime -> Wails adapter / HTTP adapter -> Go RuntimeService -> Agent Builder runtime
```
...
Agent Builder 原始 UI 是 TUI。Agent Builder 现在有 React/Wails，已经开始转向 Codex 类客户端。
...
Agent Builder 不是空白。它已经具备很多合适的底座：

- Go 实现，适合本地优先 runtime。
- `workbench/app/agent/session/message/db` 分层清楚。
- 多 provider 抽象已经存在。
- tools、MCP、skills、hooks、SQLite、LSP、TUI 都已有。
- Agent Builder 已经把 Wails/React 接到真实 Agent Builder runtime。
...
不要直接把 Claude Code 搬进 Agent Builder。应该把 Claude Code 拆成 runtime primitives，再逐步用 Go/Agent Builder 风格实现：
...
这样 Agent Builder 才会从“Agent Builder 桌面壳”变成真正接近 Codex/Claude Code 的客户端。
