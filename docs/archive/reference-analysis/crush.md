...
Crush is a terminal-first AI coding assistant, but its internals are already
...
that Crush already has a model loop, tool governance, session persistence,
...
The implementation is Go, module `github.com/charmbracelet/crush`. Core
...
through `internal/workspace`, `internal/server`, `internal/backend`, and
`internal/client`.
...
`CRUSH_PROFILE` is set, then calls `cmd.Execute()` (`main.go:22`).
...
- Client/server mode is gated by `CRUSH_CLIENT_SERVER` (`internal/cmd/root.go:212`),
...
Non-interactive execution is a parallel entry point. `crush run` reads prompt
...
Crush delegates model protocol differences to `fantasy`, but builds providers
itself from Crush config. `Coordinator.Run` refreshes models every turn,
...
kill, Crush info/logs, MCP resource tools, and the `agent` delegation tool
...
MCP tools are adapted into Crush tools and permission-gated like built-ins
(`internal/agent/tools/mcp-tools.go:24`, `internal/agent/tools/mcp-tools.go:99`).

Skills implement the Agent Skills `SKILL.md` convention. Crush parses YAML
...
Crush has a real subagent primitive, but it is narrow. Default config defines a
...
platforms and injects `CRUSH=1`, `AGENT=crush`, and `AI_AGENT=crush` into shell
...
Client/server mode creates a separate long-lived Crush server over Unix socket
...
as `.github/copilot-instructions.md`, `CLAUDE.md`, `GEMINI.md`, `CRUSH.md`, and
...
Logs go to `.crush/logs/crush.log`, with debug HTTP clients available for
provider calls. `CRUSH_PROFILE` starts pprof on localhost:6060 (`main.go:22`).
...
architecture, terminal, shell, Crush version, Go version, non-interactive mode,
and continuation flags (`internal/event/event.go:50`, `internal/event/event.go:80`).
Metrics are disabled by `CRUSH_DISABLE_METRICS`, `DO_NOT_TRACK`, or config
...
Crush is a good base for an agentic operations client if treated as a runtime
