# Legacy Agent Builder Inventory
...
The Agent Builder TUI/terminal UI surface has been removed from the active tree.
...
stub for `main.go`. Runtime event subscriptions no longer expose Bubble Tea
types; app/workbench/runtime consumers use raw `pubsub.Event[any]` payloads and
...
| legacy | Keep temporarily for Agent Builder CLI/TUI compatibility, but keep out of the client runtime path. |
...
The repository still carries Agent Builder naming and compatibility stubs, but the
active product path is no longer the original Agent Builder CLI/TUI application. The
...
| `README.md` | migrate | Mostly original Agent Builder README plus Agent Builder desktop notes. It mixes product docs, CLI docs, config docs, and desktop build notes. | Split into Agent Builder README plus legacy Agent Builder reference docs. |
...
| `main.go` | legacy | Root CLI entry point for Agent Builder command tree. Not part of desktop runtime path. | Keep until CLI adapter boundary exists; later move under `internal/adapters/cli` or a `cmd/` package if the CLI remains supported. |
...
| `Taskfile.yaml` | migrate | Still centered on Agent Builder CLI tasks (`build`, `run`, schema, release), but also contains useful project tasks. | Split or rename tasks into product, CLI legacy, and generated-code sections. |
| `agent-builder.json` | keep | Current project-level Agent Builder config for gopls/LSP. Useful while the repo is still developed with Agent Builder. | Keep for now; document that it is a developer config, not Agent Builder runtime config. |
| `schema.json` | migrate | Generated Agent Builder config schema from `go run main.go schema`; not runtime-critical for the desktop client. | Move to `docs/reference/` or generate on demand once config ownership is clarified. |
...
| `.goreleaser.yml` | archive | Upstream Agent Builder release pipeline: packages `agent-builder`, completions, man pages, AUR/Homebrew/NPM/etc. Not suitable for Agent Builder desktop releases. | Phase 7 retained: GitHub release/snapshot workflows still reference GoReleaser. Archive after release workflows are replaced. |
| `flake.nix`, `flake.lock`, `.envrc` | archive | Nix dev shell is useful only if the team actively uses it; current metadata still says "Agent Builder development environment". | Phase 7 retained: team/dev-shell usage is unconfirmed, so these were not moved. Update or archive after ownership is confirmed. |
...
| `desktop/go.mod`, `go.sum` | legacy | Independent desktop Go module retained for a conservative Phase 2 move; it now uses `replace github.com/CIPFZ/agent-builder => ..`. | Remove in a later pass after desktop builds from the root module. |
...
## Removed CLI/TUI And Legacy Agent Builder Surface
...
| `internal/commands/` | delete | Removed terminal slash/custom command parser. MCP prompt retrieval moved to runtime/workspace runtime paths. | Rebuild any future command palette as client/runtime-owned behavior. |
| `internal/workbench/events.go` | migrate | Runtime subscriptions now expose `pubsub.Event[any]` raw app events instead of Bubble Tea messages. | Continue converging on stable runtime event schemas. |
...
| `internal/server/` | legacy | HTTP/SSE transport remains for non-TUI runtime/workbench access. | Keep until local HTTP/SSE runtime adapter supersedes it. |
...
| `internal/apitypes/`, `internal/swagger/` | legacy | Protocol/OpenAPI surface for retained server APIs. | Keep generated docs aligned with removed TUI config endpoints. |
...
| `docs/legacy-agent-builder-inventory.md` | keep | Historical inventory plus current legacy status notes. | Keep as reference and update only when legacy surfaces change. |
...
| `docs/agent-builder-claude-code-gap-analysis.md` | archive | Reference comparison, not an active product contract. | Phase 7 archived to `docs/archive/agent-builder-claude-code-gap-analysis.md`; active references were updated. |
| `docs/reference-analysis/` | archive | Comparative reference research. | Phase 7 archived to `docs/archive/reference-analysis/`. |
| `docs/hooks/` | migrate | User-facing Agent Builder hooks documentation. Hooks are still product-relevant but docs are Agent Builder-branded. | Move/update to `docs/reference/hooks/` or rewrite as Agent Builder hook docs. |
...
4. Replace `tea.Msg` on the client runtime path.
   ...
   `internal/workbench/events.go`. Introduce runtime-native events, then keep
