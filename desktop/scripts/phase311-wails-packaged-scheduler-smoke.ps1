...
$smokeRoot = Join-Path $runtimeDevRoot ("phase311-wails-packaged-scheduler-smoke-" + [System.Guid]::NewGuid().ToString("N"))
...
try {
  ...
  Set-Content -Path (Join-Path $smokeRoot "smoke.txt") -Value "Phase 31.1 Wails packaged scheduler smoke" -Encoding UTF8
  ...
  Write-Host ">> go test . -run TestRuntimeBridgePhase311PackagedSchedulerClickContract"
  Run "go" @("test", ".", "-run", "TestRuntimeBridgePhase311PackagedSchedulerClickContract", "-count=1") $desktopRoot
  ...
  Write-Host "Phase 31.1 Wails packaged scheduler smoke passed. Runtime root: $smokeRoot"
} finally {
