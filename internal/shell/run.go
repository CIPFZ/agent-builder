...
type RunOptions struct {
	...
	// the Agent Builder process cwd — hooks and the bash tool have different
...
// Agent Builder handler stack. Shared by the stateless [Run] entrypoint and the
...
//  1. builtins first (so Agent Builder's in-process jq wins over any PATH binary);
...
// builtinHandler returns middleware that dispatches recognized Agent Builder
