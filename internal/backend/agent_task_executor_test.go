...
func TestBackendExecuteStartedAgentTaskRoutesToWorkspaceCoordinator(t *testing.T) {
	t.Parallel()

	backend := New(context.Background(), nil, nil)
	coord := &recordingStartedTaskCoordinator{}
	backend.workspaces.Set("workspace-1", &Workspace{App: &app.App{AgentCoordinator: coord}})

	result, err := backend.ExecuteStartedAgentTask(context.Background(), "workspace-1", agent.StartedAgentTaskExecutionRequest{
		TaskID:               "task-1",
...
func TestBackendExecuteStartedAgentTaskRequiresWorkspaceCoordinator(t *testing.T) {
	t.Parallel()

	backend := New(context.Background(), nil, nil)
	backend.workspaces.Set("workspace-1", &Workspace{App: &app.App{}})

	_, err := backend.ExecuteStartedAgentTask(context.Background(), "workspace-1", agent.StartedAgentTaskExecutionRequest{TaskID: "task-1"})
	if !errors.Is(err, ErrAgentNotInitialized) {
		t.Fatalf("err = %v", err)
	}
	_, err = backend.ExecuteStartedAgentTask(context.Background(), "missing-workspace", agent.StartedAgentTaskExecutionRequest{TaskID: "task-1"})
