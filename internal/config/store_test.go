...
func TestConfigStore_ConfigPath_GlobalAlwaysWorks(t *testing.T) {
	...
	store := &ConfigStore{
		globalDataPath: "/some/global/agent-builder.json",
	}

	path, err := store.configPath(ScopeGlobal)
	require.NoError(t, err)
	require.Equal(t, "/some/global/agent-builder.json", path)
}
...
func TestConfigStore_ConfigPath_WorkspaceReturnsPath(t *testing.T) {
	...
	store := &ConfigStore{
		workspacePath: "/some/workspace/.agent-builder/agent-builder.json",
	}

	path, err := store.configPath(ScopeWorkspace)
	require.NoError(t, err)
	require.Equal(t, "/some/workspace/.agent-builder/agent-builder.json", path)
}
...
func TestGlobalWorkspaceDir(t *testing.T) {
	dir := t.TempDir()
	t.Setenv("AGENT_BUILDER_GLOBAL_DATA", dir)
