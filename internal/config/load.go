...
func PushPopAgentBuilderEnv() func() {
	var found []string
	for _, ev := range os.Environ() {
		if strings.HasPrefix(ev, "AGENT_BUILDER_") {
			pair := strings.SplitN(ev, "=", 2)
			if len(pair) != 2 {
				continue
			}
			found = append(found, strings.TrimPrefix(pair[0], "AGENT_BUILDER_"))
		}
	}
	...
	for _, ev := range found {
		os.Setenv(ev, os.Getenv("AGENT_BUILDER_"+ev))
	}
...
func (c *Config) configureProviders(store *ConfigStore, env env.Env, resolver VariableResolver, knownProviders []catwalk.Provider) error {
	...
	restore := PushPopAgentBuilderEnv()
	defer restore()
...
func (c *Config) setDefaults(workingDir, dataDir string) {
	...
	if str, ok := os.LookupEnv("AGENT_BUILDER_DISABLE_PROVIDER_AUTO_UPDATE"); ok {
		c.Options.DisableProviderAutoUpdate, _ = strconv.ParseBool(str)
	}

	if str, ok := os.LookupEnv("AGENT_BUILDER_DISABLE_DEFAULT_PROVIDERS"); ok {
...
// so an unrelated agent-builder.json placed above the project is never picked
...
func GlobalConfig() string {
	if agentBuilderGlobal := os.Getenv("AGENT_BUILDER_GLOBAL_CONFIG"); agentBuilderGlobal != "" {
		return filepath.Join(agentBuilderGlobal, fmt.Sprintf("%s.json", appName))
	}
...
func GlobalCacheDir() string {
	if agentBuilderCache := os.Getenv("AGENT_BUILDER_CACHE_DIR"); agentBuilderCache != "" {
		return agentBuilderCache
	}
...
func GlobalConfigData() string {
	if agentBuilderData := os.Getenv("AGENT_BUILDER_GLOBAL_DATA"); agentBuilderData != "" {
		return filepath.Join(agentBuilderData, fmt.Sprintf("%s.json", appName))
	}
	...
	// for windows, it should be in `%LOCALAPPDATA%/agent-builder/`
	// for linux and macOS, it should be in `$HOME/.local/share/agent-builder/`
	if runtime.GOOS == "windows" {
		localAppData := cmp.Or(
			os.Getenv("LOCALAPPDATA"),
			filepath.Join(os.Getenv("USERPROFILE"), "AppData", "Local"),
...
// fallback keeps Agent Builder from silently adopting state files placed above
// the current project.
...
func GlobalSkillsDirs() []string {
	if agentBuilderSkills := os.Getenv("AGENT_BUILDER_SKILLS_DIR"); agentBuilderSkills != "" {
		return []string{agentBuilderSkills}
	}
	...
	// On Windows, also load from app data on top of `$HOME/.config/agent-builder`.
...
// ProjectSkillsDir returns the default project directories for which Agent Builder
// will look for skills.
func ProjectSkillsDir(workingDir string) []string {
	return []string{
		filepath.Join(workingDir, ".agents/skills"),
		filepath.Join(workingDir, ".agent-builder/skills"),
