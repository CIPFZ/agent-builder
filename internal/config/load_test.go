...
import (
	...
	"github.com/CIPFZ/agent-builder/internal/env"
...
func TestLookupConfigs_BoundedByProject(t *testing.T) {
	...
	globalDir := t.TempDir()
	t.Setenv("AGENT_BUILDER_GLOBAL_CONFIG", globalDir)
	t.Setenv("AGENT_BUILDER_GLOBAL_DATA", globalDir)

	t.Run("does not pick up agent-builder.json above non-git project", func(t *testing.T) {
		parent := t.TempDir()

		// agent-builder.json above the project must not be adopted.
		require.NoError(t, os.WriteFile(
			filepath.Join(parent, "agent-builder.json"),
			[]byte(`{}`),
			0o644,
		))
		...
		for _, p := range got {
			require.NotEqual(t, filepath.Join(parent, "agent-builder.json"), p)
		}
	})

	t.Run("does not climb out of git worktree to find agent-builder.json", func(t *testing.T) {
		...
		strayEval, err := filepath.EvalSymlinks(filepath.Join(parent, "agent-builder.json"))
		require.NoError(t, err)
		for _, p := range got {
			...
			require.NotEqual(t, strayEval, pEval, "must not adopt parent agent-builder.json")
		}
	})

	t.Run("picks up agent-builder.json inside the project", func(t *testing.T) {
		project := t.TempDir()
		local := filepath.Join(project, "agent-builder.json")
		...
		require.True(t, foundLocal, "expected project agent-builder.json to be in lookup result: %v", got)
	})
...
func TestConfig_setDefaults(t *testing.T) {
	t.Run("sets default data directory", func(t *testing.T) {
		...
		require.Equal(t, filepath.Join(workingDir, ".agent-builder"), cfg.Options.DataDirectory)
	...
	t.Run("does not adopt .agent-builder from a parent project", func(t *testing.T) {
		parent := t.TempDir()

		// .agent-builder in the parent: it should not be reused by the child
	...
	t.Run("does not climb out of git worktree to find .agent-builder", func(t *testing.T) {
		if _, err := exec.LookPath("git"); err != nil {
			t.Skip("git not available")
		}
		...
		// Stray .agent-builder above the worktree root.
		require.NoError(t, os.Mkdir(filepath.Join(parent, defaultDataDirectory), 0o755))
		...
		require.NoError(t, err)
		require.NotEqual(t, strayEval, gotEval, "must not adopt parent .agent-builder")
...
func TestConfig_setupAgentsWithDisabledTools(t *testing.T) {
	...
	assert.Equal(t, []string{"agent", "bash", "agent_builder_info", "agent_builder_logs", "job_output", "job_kill", "multiedit", "lsp_diagnostics", "lsp_references", "lsp_restart", "fetch", "agentic_fetch", "task_list", "task_get", "task_message", "task_stop", "task_output", "glob", "ls", "sourcegraph", "todos", "view", "write", "list_mcp_resources", "read_mcp_resource"}, coderAgent.AllowedTools)
...
func TestConfig_setupAgentsWithEveryReadOnlyToolDisabled(t *testing.T) {
	...
	coderAgent, ok := cfg.Agents[AgentCoder]
	require.True(t, ok)
	assert.Equal(t, []string{"agent", "bash", "agent_builder_info", "agent_builder_logs", "job_output", "job_kill", "download", "edit", "multiedit", "lsp_diagnostics", "lsp_references", "lsp_restart", "fetch", "agentic_fetch", "task_list", "task_get", "task_message", "task_stop", "task_output", "todos", "write", "list_mcp_resources", "read_mcp_resource"}, coderAgent.AllowedTools)
...
func TestConfig_setDefaultsDisableDefaultProvidersEnvVar(t *testing.T) {
	t.Run("sets option from environment variable", func(t *testing.T) {
		t.Setenv("AGENT_BUILDER_DISABLE_DEFAULT_PROVIDERS", "true")
