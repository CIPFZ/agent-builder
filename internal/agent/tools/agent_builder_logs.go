...
const AgentBuilderLogsToolName = "agent_builder_logs"

//go:embed agent_builder_logs.md.tpl
var agentBuilderLogsDescriptionTmpl []byte

var agentBuilderLogsDescriptionTpl = template.Must(
	template.New("agentBuilderLogsDescription").
		Parse(string(agentBuilderLogsDescriptionTmpl)),
)

type agentBuilderLogsDescriptionData struct {
	DefaultLines int
	MaxLines     int
}

func agentBuilderLogsDescription() string {
	return renderTemplate(agentBuilderLogsDescriptionTpl, agentBuilderLogsDescriptionData{
...
type AgentBuilderLogsParams struct {
	Lines int `json:"lines,omitempty" description:"Number of recent log entries to return (default 50, max 100)"`
}

func NewAgentBuilderLogsTool(logFile string) fantasy.AgentTool {
	return fantasy.NewAgentTool(
		AgentBuilderLogsToolName,
		agentBuilderLogsDescription(),
		func(ctx context.Context, params AgentBuilderLogsParams, call fantasy.ToolCall) (fantasy.ToolResponse, error) {
			result := runAgentBuilderLogs(logFile, params)
			return fantasy.NewTextResponse(result), nil
		},
	)
}

// runAgentBuilderLogs reads and formats the last N log entries from the given file.
func runAgentBuilderLogs(logFile string, params AgentBuilderLogsParams) string {
