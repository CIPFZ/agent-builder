...
func createTestLogFile(t *testing.T, entries []map[string]any) string {
	t.Helper()
	tempDir := t.TempDir()
	logFile := filepath.Join(tempDir, "agent-builder.log")
...
func TestNewAgentBuilderLogsTool(t *testing.T) {
	t.Parallel()
	tool := NewAgentBuilderLogsTool("/tmp/test.log")
	require.NotNil(t, tool)
	require.Equal(t, AgentBuilderLogsToolName, tool.Info().Name)
}

func TestAgentBuilderLogs_HappyPath(t *testing.T) {
	t.Parallel()
	...
	result := runAgentBuilderLogs(logFile, AgentBuilderLogsParams{Lines: 3})
...
func TestAgentBuilderLogs_DefaultLines(t *testing.T) {
	t.Parallel()
	...
	result := runAgentBuilderLogs(logFile, AgentBuilderLogsParams{Lines: 0})
...
func TestAgentBuilderLogs_MaxCap(t *testing.T) {
	t.Parallel()
	...
	result := runAgentBuilderLogs(logFile, AgentBuilderLogsParams{Lines: 200})

	lines := strings.Split(result, "\n")
	require.Len(t, lines, 100)
}

func TestAgentBuilderLogs_MissingFile(t *testing.T) {
	t.Parallel()
	result := runAgentBuilderLogs("/nonexistent/path/agent-builder.log", AgentBuilderLogsParams{Lines: 50})
	require.Contains(t, result, "No log file found")
}

func TestAgentBuilderLogs_EmptyFile(t *testing.T) {
	t.Parallel()
	tempDir := t.TempDir()
	logFile := filepath.Join(tempDir, "agent-builder.log")
	...
	result := runAgentBuilderLogs(logFile, AgentBuilderLogsParams{Lines: 50})
	require.Contains(t, result, "Log file is empty")
}

func TestAgentBuilderLogs_MalformedLines(t *testing.T) {
	t.Parallel()
	tempDir := t.TempDir()
	logFile := filepath.Join(tempDir, "agent-builder.log")
	...
	result := runAgentBuilderLogs(logFile, AgentBuilderLogsParams{Lines: 10})
...
func TestAgentBuilderLogs_ExtraFieldsSorted(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
	...
	result := runAgentBuilderLogs(logFile, AgentBuilderLogsParams{Lines: 1})
...
func TestAgentBuilderLogs_NonStringValues(t *testing.T) {
	t.Parallel()
	entry := map[string]any{
...
func TestAgentBuilderLogs_Redaction(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
...
func TestAgentBuilderLogs_ReservedFields(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
		{
			"time":   time.Now().Format(time.RFC3339),
...
func TestAgentBuilderLogs_OversizedLines(t *testing.T) {
	t.Parallel()
	tempDir := t.TempDir()
	logFile := filepath.Join(tempDir, "agent-builder.log")
...
func TestAgentBuilderLogs_PartialTrailingLine(t *testing.T) {
	t.Parallel()
	tempDir := t.TempDir()
	logFile := filepath.Join(tempDir, "agent-builder.log")
...
func TestAgentBuilderLogs_ValueQuoting(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
...
func TestAgentBuilderLogs_ChronologicalOrder(t *testing.T) {
	t.Parallel()
...
func TestAgentBuilderLogs_TimeOnlyFormat(t *testing.T) {
	t.Parallel()
	entry := map[string]any{
...
func TestAgentBuilderLogs_LevelVariations(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
	...
	result := runAgentBuilderLogs(logFile, AgentBuilderLogsParams{Lines: 5})
...
func TestAgentBuilderLogs_SourceVariations(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
