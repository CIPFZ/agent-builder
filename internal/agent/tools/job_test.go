...
func TestBackgroundShell_Kill(t *testing.T) {
	...
	bgManager := shell.GetBackgroundShellManager()
	bgShell, err := bgManager.Start(ctx, workingDir, nil, helperCommand("sleep", "100000"), "")
	require.NoError(t, err)
...
func TestBackgroundShell_ConcurrentAccess(t *testing.T) {
	...
	// Start a background shell
	bgManager := shell.GetBackgroundShellManager()
	bgShell, err := bgManager.Start(ctx, workingDir, nil, helperCommand("slow-lines"), "")
	require.NoError(t, err)
	defer bgManager.Kill(bgShell.ID)
...
func TestBackgroundShell_List(t *testing.T) {
	...
	for i := range 3 {
		bgShell, err := bgManager.Start(ctx, workingDir, nil, helperCommand("sleep", "1000"), "")
		require.NoError(t, err)
		shells[i] = bgShell
	}
...
func TestBackgroundShell_AutoBackground(t *testing.T) {
	...
	t.Run("long command stays in background", func(t *testing.T) {
		t.Parallel()
		bgManager := shell.GetBackgroundShellManager()
		bgShell, err := bgManager.Start(ctx, workingDir, nil, helperCommand("sleep", "20000")+" && echo '20 seconds completed'", "")
		require.NoError(t, err)
		defer bgManager.Kill(bgShell.ID)
