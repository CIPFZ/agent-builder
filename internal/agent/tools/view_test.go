...
func (m *mockViewPermissionService) SubscribePolicyApplications(ctx context.Context) <-chan pubsub.Event[permission.PolicyApplication] {
...
func (m mockFileTracker) MarkSessionStale(ctx context.Context, sessionID, reason string) error {
	return nil
}
...
func TestReadBuiltinFile(t *testing.T) {
	...
	t.Run("reads agent-builder-config skill", func(t *testing.T) {
		t.Parallel()

		resp, err := readBuiltinFile(ViewParams{
			FilePath: "agent-builder://skills/agent-builder-config/SKILL.md",
		}, nil)
		require.NoError(t, err)
		require.NotEmpty(t, resp.Content)
		require.Contains(t, resp.Content, "Agent Builder Configuration")
	})
	...
	t.Run("not found", func(t *testing.T) {
		...
		resp, err := readBuiltinFile(ViewParams{
			FilePath: "agent-builder://skills/nonexistent/SKILL.md",
		}, nil)
		require.NoError(t, err)
		require.True(t, resp.IsError)
	})
	...
	t.Run("metadata has skill info", func(t *testing.T) {
		...
		require.Equal(t, "agent-builder-config", meta.ResourceName)
