...
func TestTruncate_OverThreshold_ReturnsPersistedOutput(t *testing.T) {
	...
	result, persisted := Truncate(content, maxChars, ".crush/results/test_id.txt")
	require.True(t, persisted)
	require.Contains(t, result, "<persisted-output>")
	require.Contains(t, result, "</persisted-output>")
	require.Contains(t, result, ".crush/results/test_id.txt")
...
func TestTruncate_ErrorInTail_IncreasesTailSize(t *testing.T) {
	...
	result, persisted := Truncate(content, maxChars, ".crush/results/test.txt")
	require.True(t, persisted)
...
func TestTruncate_EmptyContent(t *testing.T) {
	result, persisted := Truncate("", 100, ".crush/results/test.txt")
	require.False(t, persisted)
