...
func TestAgentBuilderIgnore(t *testing.T) {
	tempDir := t.TempDir()
	t.Chdir(tempDir)
	...
	// Create a .agent-builderignore file that ignores .log files
	require.NoError(t, os.WriteFile(".agent-builderignore", []byte("*.log\n"), 0o644))
...
func TestShouldExcludeFile(t *testing.T) {
	...
	// Create .agent-builderignore file
	agentBuilderIgnoreContent := "custom_ignored/\n"
	if err := os.WriteFile(filepath.Join(tempDir, ".agent-builderignore"), []byte(agentBuilderIgnoreContent), 0o644); err != nil {
		t.Fatalf("Failed to create .agent-builderignore: %v", err)
	}
	...
	require.True(t, ShouldExcludeFile(tempDir, customIgnored), "Expected custom_ignored to be ignored by .agent-builderignore")
...
func TestShouldExcludeFileHierarchical(t *testing.T) {
	...
	// Create .agent-builderignore in subdir that ignores normal_nested
	subAgentBuilderIgnore := "normal_nested/\n"
	if err := os.WriteFile(filepath.Join(subDir, ".agent-builderignore"), []byte(subAgentBuilderIgnore), 0o644); err != nil {
		t.Fatalf("Failed to create subdir .agent-builderignore: %v", err)
	}

	// Test hierarchical ignore behavior - this should work because the .agent-builderignore is in the parent directory
	require.True(t, ShouldExcludeFile(tempDir, nestedNormal), "Expected normal_nested to be ignored by subdir .agent-builderignore")
