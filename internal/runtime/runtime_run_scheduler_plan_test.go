...
func TestRuntimeRunSchedulerPlanTaskItemAfterCancellationStaysNonExecutableAndDoesNotMutateScope(t *testing.T) {
	t.Parallel()
	...
	if _, err := service.CancelAgentTask(context.Background(), task.ID); err != nil {
		t.Fatal(err)
	}
	...
	if item.CancellationScope == "" || item.DiagnosticsRoute == "" || len(item.RefreshTargets) == 0 {
		t.Fatalf("cancelled task routing = %#v", item)
	}
	scope := item.TaskScope
	if scope.Role != task.Role || scope.Provider != task.Provider || scope.Model != task.Model || scope.CWD != task.CWD || scope.Worktree != task.Worktree || scope.ParentToolCallID != task.ParentToolCallID || scope.ChildSessionID != task.ChildSessionID {
		t.Fatalf("cancelled task scope = %#v task = %#v", scope, task)
	}
	if len(scope.AllowedTools) != 2 || scope.AllowedTools[0] != "view" || scope.AllowedTools[1] != "grep" {
		t.Fatalf("allowed tools widened or reordered = %#v", scope.AllowedTools)
	}
	...
	if refreshed.Status != agentTaskStatusCancelled || refreshed.Progress != 100 || refreshed.Role != task.Role || refreshed.CWD != task.CWD || len(refreshed.ArtifactRefs) != 0 {
		t.Fatalf("cancelled task evidence mutated by plan = %#v", refreshed)
	}
}
