...
func TestRuntimeReactCallchainFinalEmptyAfterToolResultExplainsDelivery(t *testing.T) {
	resp := buildRuntimeReactCallchain(runtimeReactCallchainInput{
		...
		messages: []RuntimeMessage{
			...
			{ID: "msg-tool", SessionID: "session-1", Role: "tool", CreatedAt: 1200, Parts: []RuntimeMessagePart{{Type: "tool_result", ToolCallID: "tool-1", Name: "bash", Content: "<persisted-output>preview</persisted-output>", DeliveredToModel: true, DeliveredAtStep: 2, DeliveryReason: "included_in_model_input", StoredPath: "runtime://objects/tool-result-1", OriginalSize: 64000, TruncatedBy: "single"}}},
