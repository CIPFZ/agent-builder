...
description: Use when the user needs help configuring Agent Builder — working with agent-builder.json, setting up providers, configuring LSPs, adding MCP servers, managing skills or permissions, or changing Agent Builder behavior.
---

# Agent Builder Configuration

Agent Builder uses JSON configuration files with the following priority (highest to lowest):

1. `.agent-builder.json` (project-local, hidden)
2. `agent-builder.json` (project-local)
3. `$XDG_CONFIG_HOME/agent-builder/agent-builder.json` or `$HOME/.config/agent-builder/agent-builder.json` (global)

## Basic Structure

```json
{
  "$schema": "https://github.com/CIPFZ/agent-builder/schema.json",
  "models": {},
  "providers": {},
  "mcp": {},
  "lsp": {},
...
Agent Builder runs selected string fields through an embedded bash-compatible
...
`agent-builder.json` is trusted code. Any `$(...)` in it runs at load time
with the invoking user's shell privileges, before the UI appears.
Don't launch Agent Builder in a directory whose `agent-builder.json` you haven't
reviewed.
...
```json
{
  "options": {
    ...
    "disabled_skills": ["agent-builder-config"],
    "auto_lsp": true,
    "debug": false,
    "debug_lsp": false,
    "attribution": {
...
> `.agents/skills`, `.agent-builder/skills`, `.claude/skills`, `.cursor/skills`
    ...
    "PreToolUse": [
      {
        ...
        "command": ".agent-builder/hooks/protect-files.sh"
      },
      {
        "matcher": "^bash$",
        "command": ".agent-builder/hooks/no-haskell.sh"
      }
    ]
  }
}
...
|---|---|
| `AGENT_BUILDER_EVENT` | Event name (e.g. `PreToolUse`) |
| `AGENT_BUILDER_TOOL_NAME` | Name of the tool being called |
| `AGENT_BUILDER_SESSION_ID` | Current session ID |
| `AGENT_BUILDER_CWD` | Current working directory |
| `AGENT_BUILDER_PROJECT_DIR` | Project root directory |
| `AGENT_BUILDER_TOOL_INPUT_COMMAND` | Value of `command` from tool input (if present) |
| `AGENT_BUILDER_TOOL_INPUT_FILE_PATH` | Value of `file_path` from tool input (if present) |
...
Agent Builder also supports the Claude Code hook output format:
...
- `AGENT_BUILDER_GLOBAL_CONFIG` - Override global config location
- `AGENT_BUILDER_GLOBAL_DATA` - Override data directory location
- `AGENT_BUILDER_SKILLS_DIR` - Override default skills directory
