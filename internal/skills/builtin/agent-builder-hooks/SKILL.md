---
name: agent-builder-hooks
description: Use when the user wants to add, write, debug, or configure a Agent Builder hook — gating or blocking tool calls, approving or rewriting tool input before execution, injecting context into tool results, or troubleshooting hook behavior in agent-builder.json.
---

# Agent Builder Hooks

Hooks are user-defined commands in `agent-builder.json` that fire at specific points
...
For the runtime capability overview, see `docs/data-and-capabilities.md`. This skill covers what you
...
| `AGENT_BUILDER_TOOL_NAME`            | Tool being called (e.g. `bash`)          |
...
| `AGENT_BUILDER_TOOL_INPUT_COMMAND`   | For `bash` calls: the shell command      |
...
3. Add the entry under `hooks.PreToolUse` in `agent-builder.json` with the right matcher.
...
- Non-zero exit codes other than 2/49 are logged but don't block — check Agent Builder logs.
...
Agent Builder also accepts Claude Code's `hookSpecificOutput` envelope. One intentional
divergence: Agent Builder treats `updated_input` as shallow-merge, Claude Code replaces.
